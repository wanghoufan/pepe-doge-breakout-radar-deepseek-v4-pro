...
- **成败样本与回测**：全量历史突破扫描 217 个（成功 75 / 失败 142）+ 普通行情对照窗口，walk-forward 样本外回测，如实呈现「朴素突破信号≈无超额」的结论。
...
- 样本量有限：仅两个标的、约 883 天 4H 数据、217 个突破，walk-forward 各 fold 样本稀疏，Precision/Recall 波动大。
