...
- 历史维度已纳入失败样本与普通行情对照窗口（冻结样本基线：Raw 层 290 / Episode 层 116 独立事件〈成功 41 / 失败 75〉/ Normal 层 768 窗口〈760 可评〉；旧「217（成功 75 / 失败 142）」为旧 raw-cooldown 口径、已作废），用于 walk-forward 样本外检验，不再只展示人工挑选的成功案例。
...
- 不谈面向交易决策的收益/准确性/胜率；回测指标仅作研究诚实性证据、不构成收益承诺；研究指标（误报率/Precision/Recall/FPR/Success 率）仅离线研究用，不入 ActionCard/报警、不包装成胜率；失败样本与普通行情对照窗口已纳入（V2），在 UI 与方法论里如实呈现。
